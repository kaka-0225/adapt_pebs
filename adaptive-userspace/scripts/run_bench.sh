...
CGROUP_NAME="htmm"
###### update DIR!
DIR=/mnt/sas_ssd/lyh/memtis-Nomad/adaptive-userspace
...
function func_memtis_setting() {
    ...
    echo 3 | tee /sys/kernel/mm/htmm/htmm_thres_hot
...
function func_prepare() {
	...
	# Allow perf NMI to use up to 25% CPU time before throttling.
	# Do NOT set to 0 (disables throttle entirely, can cause hard lockup
	# when ksamplingd floods the system with PEBS NMIs).
	sudo sysctl kernel.perf_cpu_time_max_percent=25
...
function func_main() {
    ...
    # Pin to Node 0 CPUs only (28 cores: even-numbered 0-54)
    PINNING="numactl --cpunodebind=0 --"
    ...
    # 清理上次运行残留的追加文件（防止 memory_stat.sh 的 >> 污染数据）
    rm -f ${LOG_DIR}/hotness_stat.txt ${LOG_DIR}/memory_stat.txt \
          ${LOG_DIR}/pgmig.txt ${LOG_DIR}/numastat_periodic.txt \
          ${LOG_DIR}/cgroup_numa_stat.txt
    ...
    # 诊断：验证 DRAM 配额是否正确写入 cgroup
    CGDRAM_FILE="/sys/fs/cgroup/htmm/memory.max_at_node0"
    if [[ -f "${CGDRAM_FILE}" ]]; then
        CGDRAM_BYTES=$(cat "${CGDRAM_FILE}")
        CGDRAM_MB=$(echo "${CGDRAM_BYTES}/1024/1024" | bc)
        echo "[HTMM_DIAG] memory.max_at_node0 = ${CGDRAM_BYTES} bytes (${CGDRAM_MB} MB)"
        echo "[HTMM_DIAG] memory.htmm_enabled = $(cat /sys/fs/cgroup/htmm/memory.htmm_enabled 2>/dev/null || echo 'N/A')"
        echo "[HTMM_DIAG] cgroup.procs size = $(wc -l < /sys/fs/cgroup/htmm/cgroup.procs)"
    else
        echo "[HTMM_DIAG] WARNING: ${CGDRAM_FILE} 不存在 — htmm cgroup 可能未正确创建！"
    fi
    ...
    # dmesg: 如果调用者设置了 SKIP_DMESG_CLEAR，则不清空 buffer（由调用者管理）
    if [[ "${SKIP_DMESG_CLEAR:-}" != "1" ]]; then
