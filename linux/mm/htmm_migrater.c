				       ...
				       pg_data_t *pgdat, bool promotion)
{
	...
	/* ============================================================
     * Test8 迁移追踪: 在迁移执行前记录每个页面的虚拟地址和方向
     *
     * 对于匿名页面 (MAP_ANONYMOUS | MAP_PRIVATE):
     *   vm_pgoff = vm_start >> PAGE_SHIFT
     *   page->index = (vaddr - vm_start) / PAGE_SIZE + vm_pgoff
     *              = vaddr >> PAGE_SHIFT
     * 因此: vaddr = page->index << PAGE_SHIFT
     *
     * trace格式: [MIGRATE] dir=P/D va=0x... src=X dst=Y
     *   P = promote (PM→DRAM), D = demote (DRAM→PM)
     *
     * 注意: 此追踪会增加迁移路径开销, 仅用于实验分析
     * 删除时: 移除从此注释到 "Test8 迁移追踪结束" 之间的代码
     * ============================================================ */
	{
		struct page *__page;
		list_for_each_entry (__page, migrate_list, lru) {
			if (PageAnon(__page)) {
				      ...
				      unsigned long nr_to_reclaim)
{
	...
	while (!list_empty(page_list)) {
		...
		if (PageTransHuge(page) && !thp_migration_supported())
			goto keep_locked;
		if (!PageAnon(page) &&
		    nr_demotion_cand > nr_to_reclaim + HTMM_MIN_FREE_PAGES)
			goto keep_locked;
		...
		if (!file) {
			...
			if (PageTransHuge(compound_head(page))) {
				...
				if (check_split_huge_page(memcg,
							  ...
							  false)) {
					spin_lock_irq(&lruvec->lru_lock);
					if (deferred_split_huge_page_for_htmm(
						    compound_head(page))) {
						spin_unlock_irq(
							&lruvec->lru_lock);
...
static int kmigraterd_demotion(pg_data_t *pgdat)
{
	...
	for (;;) {
		...
		if (htmm_thres_split != 0 &&
			...
			nr_split =
				deferred_split_scan_for_htmm(pn, &split_list);
